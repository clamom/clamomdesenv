...
{	
	if($('#countpag').val() > 0)//depois 1° desmembramento 
	{
		$('#add').removeAttr("disabled");
		$('#add').removeClass("disabled");
		$('#save_completo').removeAttr("disabled");
		$('#save_completo').removeClass("disabled");
	}
	if($('#countpag').val() > 1)//depois não liberar mais os botoes Apagar, Adicionar, Salvar Grafico
	{
		if($('#countpag').val() == 2)
		{
			$('#dele').removeAttr("disabled");
			$('#dele').removeClass("disabled");
		}
		else
		{
			$('#dele').attr("disabled",true);
			$('#dele').addClass("disabled");
		}
		$('#add').attr("disabled",true);
		$('#add').addClass("disabled");
		$('#save_completo').attr("disabled",true);
		$('#save_completo').addClass("disabled");
	}
	usuario = $("#usuario").val();
	d = new Date();
	month = d.getMonth()+1;
	...
	$('td').dblclick(function(e){
		...
		//alert( countpag+'  '+corFonte )
		if(idsubitem>0)
		{
			if(parseInt(countpag) <= 1 || corFonte == "color:#FF0000")
			{
	...
	$('.codigo').live("click",function(e){//selecionar filas pintar amarelo
		...
		if(fila > 0) 
		{
			...
			success: function(html)
			{
				if(html == "revisao")//mostrar revisão
				{
		...
		if($("#AllCodigo").attr("checked"))
		{
			...
			if(fila > 0)
			{
				...
				success: function(html)
				{
					if(html == "revisao")
					{
						//$('#finalizar').attr("disabled");//ocultar
						//$('#finalizar').addClass("disabled");//ocultar
						$('#revisao').removeAttr("disabled");//mostrar
						$('#revisao').removeClass("disabled");//mostrar
					}
					else
					{
						//$('#finalizar').removeAttr("disabled");//mostrar
						//$('#finalizar').removeClass("disabled");//mostrar
	...
	$('#liberacao').click(function(){
		...
		if(fila > 0)
		{
			...
			if(e)
			{
				...
				type: "POST",
				url: "ajax/ajax_projeto.php",
				data: {data:codigos,usuario:usuario,tipo:tipo,num_ctr:num_ctr,id_ctr:id_ctr,op:"liberacao"},
				cache: false,
				success: function(html)
				{
					$('#save_completo').click();
