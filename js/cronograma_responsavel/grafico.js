...
{	
    ...
    $('td').live("mouseup",function(e){
		...
		case 3://alert('click right');
			if(!$('#save_realizado').is(':visible'))
			{
				...
				if($(this).attr("tarefa")>0)/*PARA PEGAR TAREFA NIVEL ACIMA ^|^ */
				{
									...
									if(corFonte == "color:#333333")//não adiciono etapas
									{
										...
										{							
											...
											$.ajax({
												...
												success: function(html)
												{
													window.location.href = "index.php?projeto="+projeto+"&item=0&subitem="+idsubitem+"&tipo="+tipo+"&tipo2="+tipo2+"&gere_coor="+gere_coor+"&usuario="+usuario;
												}
											});
										}
										});
	...
	$('.liTarefa').live("click",function(e){
		...
		if(tarefa != "")
		{
			...
			{
				estado = html;
				if(estado == "0")//finalizar	if($('#finalizar').is(':visible'))
				{
	...
	$('#add').click(function(e){//adicionar subitens
		...
		var dataString = 'id_ctr='+id_ctr+'&id_item='+id_item+'&id_subitem='+id_subitem+'&num_ctr='+num_ctr+'&countpag='+countpag+'&usuario='+usuario+'&tipo2='+tipo2+'&op=left';
		$.ajax({
			type: "POST",
	...
	$('.codigo').live("click",function(e){//selecionar filas pintar amarelo
		...
		if(fila > 0) 
		{
			/*if(countpag >= 2)//mostrar botão de liberação
			{
				if(var1 == "true")
				{
					$('#liberacao').removeAttr("disabled");
					$('#liberacao').removeClass("disabled");
				}
				else//ocultar botão de liberação
				{
					$('#liberacao').attr("disabled");
					$('#liberacao').addClass("disabled");
				}
			}
			else//ocultar botão de liberação
			{
			...
			success: function(html)
			{
				if(html == "false")//sem tarefas
				{
					...
					$('#liberacao').attr("disabled",true);/*+-*/
					$('#liberacao').addClass("disabled");/*+-*/
					alertify.alert("Adicionar tarefas e clicar em [Salvar Grafico]");
				}
				else if(html == "finalizar")//mostrar revisão
				{
				...
				else if(html == "liberar") //mostrar finalizar
				{
					$('#finalizar').attr("disabled",true);
					$('#finalizar').addClass("disabled");
					$('#liberacao').removeAttr("disabled");/*+-*/
					$('#liberacao').removeClass("disabled");/*+-*/
					//$('#revisao').attr("disabled");
					//$('#revisao').addClass("disabled");
				}
			}
			});
		}
	...
	$('.save').live("click",function(e){//salvar o subitem
		...
		{
			thisSave = $(this);
			var dataString = 'valor='+valor+'&nome='+nome+'&usuario='+usuario+'&id_ctr='+id_ctr+'&id_subitem='+id_subitem;
			$.ajax({
			type: "POST",
			...
			success: function(html)
			{
				if(html == "true")
				{
					...
					thisSave.prev().prev().removeAttr("style","display:none");//mostra descricao
					thisSave.parent().children('.set_responsa').removeAttr("style","display:none");//mostra botao R(resposanvel)
					thisSave.prev().prev().html(nome);//colocar na div novo nome
				}
				else
				{
					alertify.alert('Descrição já existe <b>'+nome.toUpperCase()+'</b>, tente novamente');
				}
			}
			});
		}
		...
		if(e.which == 13){
			...
			{	
				...
				success: function(html)
				{
					if(html == "true")
					{
						thisSave.next().attr("style","display:none");//ocultar botao salvar
						thisSave.next().next().removeAttr("style","display:none");//mostrar botao editar*/
						thisSave.attr("style","display:none");//ocultar campo text
						thisSave.prev().removeAttr("style","display:none");//mostra descricao
						thisSave.next().next().next().next().removeAttr("style","display:none");//mostra botao R(resposanvel)
						thisSave.prev().html(nome);//colocar na div novo nome
					}
					else
					{
						alertify.alert('Descrição já existe <b>'+nome.toUpperCase()+'</b>, tente novamente');
	...
	$('.set_responsa').live("click",function(e)
	{
		...
		if(tipo2 == 'ger')
		{
			if($(this).val() == 'R')
			{
				$('.tipor[value=Res]').attr('checked', true);
			...
			{
				//$('.tipor[value=Res]').attr('disabled',true);
				$('.tipor[value=Exe]').attr('checked', true);
				$('#Res').attr("style","display:none;");
				$('#Exe').removeAttr("style","display:none;");
				$('#lista_resposavel').children().remove();
				$('#myResponsa').css({'top':mouseY,'left':mouseX}).fadeIn();//mostrar responsavel FASE, ETAPA E ITEM
				dataString = 'id_ctr='+id_ctr+'&id_subitem='+id_subitem+'&id_item='+id_item+'&task='+task+'&op=ListaExecutor';//listar responsaveis
				$.ajax({
					type: "POST",
					url: "ajax/ajax_menu_direito.php",
					data: dataString,
	...
	$.MarcarDesmarcar = function()//função para marcar e desmarcar os subitens
	{
		...
		if($("#AllCodigo").attr("checked"))
		{
	   	...
	   	else
	   	{
			...
			$('#finalizar').attr("disabled",true);//ocultar
			$('#finalizar').addClass("disabled");//ocultar
			$('#revisao').attr("disabled",true);//ocultar
			$('#revisao').addClass("disabled");//ocultar
			$('#liberacao').attr("disabled",true);
			$('#liberacao').addClass("disabled");
	   	}		
	}
	...
	$('#save_completo').click(function(){
		...
		else
		{
			...
			if(Atarefa.length > 0)
			{
				...
				{
					$("#msn_save").fadeOut();
					$("#copy").removeAttr("disabled",false);//habilitar botao duplicar
					$("#AllCodigo").attr("checked",false);//desmarcar checkbox
					$(".codigo").attr("checked",false);//desmarcar checkbox
					if(countpag >= 3)
					{
		...
		if(responsavel != 0)
		{
			...
			$.ajax({
				...
				success: function(html)
				{
					//if(tipor == "Res")
					//{		
						$('#'+fila_row).children().next().children('.itemResponsa').children('.divResponsavel').html(html);
					//}
	...
	/*********** TIPO RESPONSAVEL / EXECUTOR ************/
	$('.tipor').change(function(){
		if($(this).val() == "Res")
		{
			$('#Res').removeAttr("style","display:none;");
			$('#Exe').attr("style","display:none;");
			id_ctr 	   = $('#id_ctr').val();
			id_subitem = $('#id_subitem2').val();
			task       = $('#task2').val();
			id_item    = '';
			$('#lista_resposavel').children().remove();
			dataString = 'id_ctr='+id_ctr+'&id_subitem='+id_subitem+'&id_item='+id_item+'&task='+task+'&op=ListaResponsa';//listar responsaveis
			$.ajax({
				type: "POST",
				url: "ajax/ajax_menu_direito.php",
				data: dataString,
	...
	$('#finalizar').click(function(){
		...
		if(fila > 0)
		{
			...
			if(e)
			{
				...
				data: {data:codigos,usuario:usuario,tipo:tipo,id_ctr:id_ctr,num_ctr:num_ctr,op:"finalizar"},
				cache: false,
				success: function(html)
				{
					$('#save_completo').click();
	...
	$('#liberacao').click(function(){
		...
		if(fila > 0)
		{
			...
			if(e)
			{
				...
				type: "POST",
				url: "ajax/ajax_projeto.php",
				data: {data:codigos,usuario:usuario,tipo:tipo,id_ctr:id_ctr,num_ctr:num_ctr,op:"liberacao"},
				cache: false,
				success: function(html)
				{
					$('#save_completo').click();
