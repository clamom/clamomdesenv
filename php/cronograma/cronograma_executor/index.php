...
<script type="text/javascript" src="../../../js/cronograma_executor/grafico.js"></script><?php /* todas as acoes do cronograma */?>
...
<link rel="stylesheet" type="text/css" href="../../../css/cronograma_executor/grafico.css"/>
<title>Meu Cronograma [Executor]</title>
</head>
...
{	//tipo
	...
	{
		$resu7 = mysql_query("select upper(s.descricao) desc_nome,s.id_itens,s.id_itens_SubComponente,s.id idsubitem,upper(u.name) nome,s.id_tarefa,s.nr_ctr proj_ctr,concat(p.proj_ctr,' | ',p.proj_desc_ctr) titulo, s.status_finalizar estado_finalizar,s.gerar_arquivo FROM tb_projeto_sub_itens s inner join sec_users u on s.responsavel=u.login inner join tb_projeto p on p.proj_ctr=s.nr_ctr where s.id='".$id_subitem."' ");
		...
		else $titulo = "<div class='erro_geral'>ERRO NO CRONOGRAMA EXECUTOR<br/>Entrar em contato [Eduardo Zambrano][eduardoz@clamom.com.br][TI]</div>";
$resu_my = mysql_query("SELECT DAY(MIN(data_inicio)) dia1,MONTH(MIN(data_inicio)) mes1,YEAR(MIN(data_inicio)) ano1, DAY(MAX(data_final)) dia2,MONTH(MAX(data_final)) mes2, YEAR(MAX(data_final)) ano2, MIN(data_inicio) data_inicio, MAX(data_final) data_final, id_tarefa FROM tb_projeto_detalhe_tarefa WHERE id_sub_item='".$id_subitem."'");
	...
	if(count($array_link)>0)
	{
		foreach(array_reverse($array_link) as $vetor=>$v)
		{
			...
			//if($i > 4)
			//{
				if($m == 0) $navegacao .= "<img src='../../../images/cronograma_all/arrow.png' width='15' class='imgArrow' /><div class='link'>".$v["nome"]."<br/>".$v["responsavel"]."</div>"; 
				else $navegacao .= "<img src='../../../images/cronograma_all/arrow.png' width='15' class='imgArrow' /><div class='link'>".$v["nome"]."<br/>".$v["responsavel"]."</div>";
			//}
			$m++;
		}
	}
	if(status_arquivo($id_subitem) == "1")
	{
	?>
    ...
    <input type="button" id="save_realizado" value="Salvar Status" class="botao" />
    <input type="button" id="save_completo" value="Salvar Grafico" class="botao disabled" disabled="disabled" />
    <input type="button" id="gerar_arquivo" value="Gerar Arquivo" class="botaoFim disabled" style="background:#FF0000; font-weight:bold; border:#FFA3A4 solid 1px;" disabled="disabled" />
<?php	
	}
	else//realizado
	{?>
    ...
    <input type="button" id="save_realizado" value="Salvar Status" class="botao disabled" disabled="disabled" />
...
$cont=0;
	...
	if($id_subitem > 0)//SUBITEM
	{
		$resu8 = mysql_query("SELECT *,upper(descricao) descricao FROM tb_projeto_sub_itens where id_itens_SubComponente='".$id_subitem."' and executor='".$_REQUEST["executor"]."' ORDER BY num_sub_item");
	}
	...
	{	
		...
		<td width='20'><input type='checkbox' name='codigo' class='codigo' value='".$row8["id"]."' gerar_arquivo='".$row8["gerar_arquivo"]."' /></td>
		<td style='font-size:10px; font-weight:bold; text-transform:uppercase !important;' height='45'>
			...
			<input type='button' name='set_responsa' class='set_responsa ".$setresponsa_class."' value='R' ".$setresponsa." style='display:none;' />
		</td></tr>";
...
else echo "<div class='erro_geral'>ERRO NO CRONOGRAMA EXECUTOR<br>Projeto ou Item n&atilde;o encontrados <br/>Entrar em contato [Eduardo Zambrano][eduardoz@clamom.com.br][TI]</div>";
?>
<div id="myObs">
	<div class="closeMenu"><img src="../../../images/cronograma_all/x.png" /></div>
    <ul class="corpomyObs">
...
<div id="myResponsa">
    ...
    <img class="imgQuadro" src="../../../images/cronograma_all/user-icon.png" width="15" /><div class="titulo1">Designar Respons&aacute;vel</div>
    <ul>
    	<li><input type="radio" name="tipor" class="tipor" value="Res" checked="checked" /><b>Respons&aacute;vel</b></li>
