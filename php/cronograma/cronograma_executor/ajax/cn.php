...
function lista_link($id_subitem,$array,$id_ctr,$tipo,$usuario)
{
	if($tipo == "0") $tipo = 1;
	else $tipo = 0;
	...
	{
		$src   = "index.php?projeto=".$id_ctr."&item=".$row["id_itens"]."&subitem=0&tipo=1&usuario=".$_REQUEST["usuario"];
	...
	{	
		...
		$src   = "index.php?projeto=".$id_ctr."&item=0&subitem=".$row["id_itens_SubComponente"]."&tipo=".$tipo."&usuario=".$_REQUEST["usuario"];
		$array[$row["idsubitem"]] = array('nome' => $nomeItem." | ".$row["desc_nome"],'id' => $row["idsubitem"],'src' => $src,'responsavel' => $responsa);
		$array = lista_link($row["id_itens_SubComponente"],$array,$id_ctr,$tipo,$usuario);
	}	
return $array;
}
...
function status_arquivo($id_subitem)
{
	$boolena = "1";
	$resu = mysql_query("select * from tb_projeto_sub_itens where id_itens_SubComponente='".$id_subitem."' ");
	while($row = mysql_fetch_array($resu))
	{
		if($row["gerar_arquivo"] == "0")
		{
			$boolena = "0";
			break;
		}
