...
if($_REQUEST["op"]=="desmembrar")
{
	...
	if($num_detalhe > 0)
	{
		while($row_deta = mysql_fetch_array($resu_deta))
		{
			...
			$sql="insert into tb_projeto_sub_itens(id_ctr,id_itens,id_tarefa,num_sub_item,descricao,data_inicio,num_dias,data_final,login,data_cadastro,nr_ctr,id_itens_SubComponente,ordem_tarefa,responsavel,liberacao)
				values('".$_REQUEST["id_ctr"]."','".$row_item["id_itens"]."','".$row_deta["id_tarefa"]."','".$num_sub_item."','',now(),'0',now(),'".$_REQUEST["usuario"]."',now(),'".$_REQUEST["num_ctr"]."','".$IdSubComponente."','".$row_deta["ordem"]."','".$responsavel."','".$row_res["liberacao"]."')";                                  /*".strtoupper($row_desc["desc_item"])."*/
				mysql_query($sql);
