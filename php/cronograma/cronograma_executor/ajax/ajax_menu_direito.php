...
if($_REQUEST["op"]=="ListaResponsa")
{
	...
	if($_REQUEST["task"]>0)//1 NIVEL ID_ITEM > 0
	{
		...
		else//não é compras
		{
			...
			else//outros casos
			{
				...
				$listaUsuario = mysql_query("select u.login, upper(u.name) as nome, u.email, u.classificacao from sec_users_groups g inner join sec_users u on g.login=u.login where g.group_id = (select user_grupo from tb_projeto_tarefas where id='".$_REQUEST["task"]."') order by u.classificacao");
			}
		}
	}
	if(mysql_num_rows($listaUsuario)==0)//outros
	{
		...
		{
			$row  = mysql_fetch_array($resu);
			$listaUsuario = mysql_query("select u.login, upper(u.name) as nome, u.email, u.classificacao from sec_users_groups g inner join sec_users u on g.login=u.login where g.group_id = (select user_grupo from tb_projeto_tarefas where id='".$row["id_independente"]."') order by u.classificacao");
		}
		else
		{
			$listaUsuario = mysql_query("select u.login, upper(u.name) as nome, u.email, u.classificacao from sec_users u inner join sec_users_groups g on u.login=g.login where g.group_id in (1,5,14,20,22,21,24,31,42,43) order by u.classificacao, u.name");
...
//$listaUsuario = mysql_query("select u.login, upper(u.name) as nome, u.email from sec_users u inner join sec_users_groups g on u.login=g.login where g.group_id=(SELECT group_id FROM sec_users_groups where login=(SELECT proj_responsavel FROM tb_projeto where id='".$_REQUEST["id_ctr"]."' )) order by u.name");
//$listaUsuario = mysql_query("select u.login, upper(u.name) as nome, u.email from sec_users u inner join sec_users_groups g on u.login=g.login where g.group_id=(SELECT group_id FROM sec_users_groups where login=(SELECT proj_responsavel FROM tb_projeto where id='".$_REQUEST["id_ctr"]."' )) order by u.name");
if($_REQUEST["op"]=="SaveResponsavel")
{
	...
	while($row1 = mysql_fetch_array($resu1))
	{
		if($row["responsavel"] == $row1["responsavel"])
		{
			...
			{
				$sql="update tb_projeto_sub_itens set responsavel='".$vetor["1"]."',login_alteracao='".$_REQUEST["usuario"]."',data_alteracao=now() where id='".$row1["id"]."' ";
			}
			else
			{
	...
	if($_REQUEST["tipor"] == "Res")
	{
		$sql="update tb_projeto_sub_itens set responsavel='".$vetor["1"]."',login_alteracao='".$_REQUEST["usuario"]."',data_alteracao=now() where id='".$vetor["0"]."' ";
	}
	else
	{
...
if($_REQUEST["op"]=="fim_rev")
{
	...
	foreach($vetor as $v)
	{
		...
		{ 
			$liberar_arquivo = "false";
			break;
		}
		elseif($v[0]=="1")//com tarefas
		{	//finalizar 
			if($v[1]=="0")//pendente liberar arquivo
			{
				$liberar_arquivo   = "true";
			}
			elseif($v[1]=="1")//arquivo já foi liverado
			{
				$liberar_arquivo   = "";
			}
		}
	}
	if($liberar_arquivo == "false")
	{
		echo "false";
	}
	elseif($liberar_arquivo == "true")
	{
		echo "liberar";
	}
	else
