<!--
 * @CRONOGRAMA GERAL [SUBITEM | SUBITEM | ...]
...
<script type="text/javascript" src="../../../../js/cronograma_geral/grafico_subitem.js"></script><?php /* todas as acoes do cronograma */?>
...
<link rel="stylesheet" type="text/css" href="../../../../css/cronograma_geral/grafico_subitem.css"/>
...
{	//tipo
	...
	{
		$estado_finalizar = 0;
		$resu7 = mysql_query("select upper(i.desc_item) desc_nome,p.proj_ctr,p.proj_status_finalizar,i.responsavel,concat(p.proj_ctr,' | ',p.proj_desc_ctr) titulo,i.num_item,i.quantidade FROM tb_projeto p inner join tb_projeto_itens i on p.id=i.projeto_id where i.id='".$id_item."' and p.proj_situacao <> 'Finalizado'");
		if (mysql_num_rows($resu7)>0)    
		{
			$row7     = mysql_fetch_array($resu7);
			$titulo   = $row7["titulo"];
			//pegar responsavel
			$resuRI   = mysql_query("select *,upper(name) nome from sec_users where login='".$row7["responsavel"]."' ");
			$rowRI    = mysql_fetch_array($resuRI);
			//dados sigla => responsavel
			$resu_gru = mysql_query("select * from sec_users_groups where login='".$row7["responsavel"]."' ");
			$row_gru  = mysql_fetch_array($resu_gru);
			$idcodigo = return_mar($row_gru["group_id"]);
			$resu_sigla2 = mysql_query("select * from tb_projeto_tarefas where id='".$idcodigo."' ");
			$row_sigla2  = mysql_fetch_array($resu_sigla2);
			$responsa    = "[".($row_sigla2["sigla_tarefa"])."][R. ".$rowRI["name"]."]";
			//$responsa = "[R. ".$rowRI["name"]."]";
			$nomeItem    = "Nº ".$row7["num_item"]." ITEM | ".$row7["desc_nome"];
			$qtdItem     = "Quantidade ".$row7["quantidade"];
		}
		else $titulo = "<div class='erro_geral'>ERRO NO CRONOGRAMA SUBITEM <br/>Entrar em contato [Eduardo Zambrano][eduardoz@clamom.com.br][TI]</div>";
//$resu_my = mysql_query("SELECT DAY(data_inicio) dia1,MONTH(data_inicio) mes1,YEAR(data_inicio) ano1,DAY(data_final) dia2,MONTH(data_final) mes2, YEAR(data_final) ano2,data_inicio,data_final FROM tb_projeto_itens WHERE id='".$id_item."'");
$resu_my = mysql_query("SELECT DAY(MIN(ta.data_inicio)) dia1,MONTH(MIN(ta.data_inicio)) mes1,YEAR(MIN(ta.data_inicio)) ano1, DAY(MAX(ta.data_final)) dia2,MONTH(MAX(ta.data_final)) mes2, YEAR(MAX(ta.data_final)) ano2, MIN(ta.data_inicio) data_inicio, MAX(ta.data_final) data_final FROM cro_projeto_item_tarefa ta inner join cro_projeto_etapa_item ei on ta.id_etapa_item = ei.id_etapa_item WHERE ei.id_item = '".$id_item."'");
		//navegação fase e etapa
		$resu_fase = mysql_query("select * from cro_projeto_etapa_item where id_item = '".$id_item."'");
	...
	if($m==0) $navegacao_fase .= "<div class='link'><a href='../".$v["src"]."'>".$v["nome"]."<br/>".$v["responsavel"]."</a></div>";
			  ...
			  $navegacao_fase .= "<img src='../../../../images/cronograma_all/arrow.png' width='15' class='imgArrow' /><div class='link'><a href='../index.php?projeto=".$id_ctr."&id_fase=".$row_fase["id_etapa"]."&usuario=".$_REQUEST["usuario"]."'>".$nomeItem."<br>".$responsa."</a></div>";
			}
		}
		unset($array_link);
 	}
	elseif($id_subitem > 0)//SUBITEM
	{
		$resu7 = mysql_query("select upper(s.descricao) desc_nome,s.id_itens,s.id_itens_SubComponente,s.id idsubitem,upper(u.name) nome,s.id_tarefa,s.nr_ctr proj_ctr,concat(p.proj_ctr,' | ',p.proj_desc_ctr) titulo, s.status_finalizar estado_finalizar FROM tb_projeto_sub_itens s inner join sec_users u on s.responsavel=u.login inner join tb_projeto p on p.proj_ctr=s.nr_ctr where s.id='".$id_subitem."' ");
		if (mysql_num_rows($resu7)>0)
		{
    		$row7       = mysql_fetch_array($resu7);
...
$resu_my = mysql_query("SELECT DAY(MIN(data_inicio)) dia1,MONTH(MIN(data_inicio)) mes1,YEAR(MIN(data_inicio)) ano1, DAY(MAX(data_final)) dia2,MONTH(MAX(data_final)) mes2, YEAR(MAX(data_final)) ano2, MIN(data_inicio) data_inicio, MAX(data_final) data_final, id_tarefa FROM tb_projeto_detalhe_tarefa WHERE id_sub_item='".$id_subitem."'");
	...
	if($m==0) $navegacao_fase .= "<div class='link'><a href='../".$v["src"]."'>".$v["nome"]."<br/>".$v["responsavel"]."</a></div>";
			...
			$navegacao_fase .= "<img src='../../../../images/cronograma_all/arrow.png' width='15' class='imgArrow' /><div class='link'><a href='../index.php?projeto=".$id_ctr."&id_fase=".$row_fase["id_etapa"]."&usuario=".$_REQUEST["usuario"]."'>".$nomeItem."<br>".$responsame1."</a></div>";
			}
		}
		unset($array_link);
		//navegação principal
		...
		{
			$src="index.php?projeto=".$id_ctr."&item=".$row7["id_itens"]."&subitem=0&tipo=1&usuario=".$_REQUEST["usuario"];
		...
		{
			$tipo1 = $tipo;//tipo 
			$src = "index.php?projeto=".$id_ctr."&item=0&subitem=".$row7["id_itens_SubComponente"]."&tipo=".$tipo1."&usuario=".$_REQUEST["usuario"];
			$array_link[$row7["id"]] = array('nome' => $nomeItem." | ".$nomeSubItem,'id' => $row7["id"],'src' => $src,'responsavel' => $responsa);
			$array_link = lista_link($row7["id_itens_SubComponente"],$array_link,$id_ctr,$tipo1,$_REQUEST["usuario"]);
		}
	}
	if(count($array_link)>0)
	{
	...
	//if($row7["proj_status_finalizar"]==0)//cronograma
...
<?php	
	...
	{?>
    <input type="button" id="save_realizado" value="Salvar Executor" class="botao disabled" disabled="disabled" />
...
$cont=0;
	...
	elseif($id_subitem > 0)//SUBITEM
	{
		$resu8 = mysql_query("SELECT *,upper(descricao) descricao FROM tb_projeto_sub_itens where id_itens_SubComponente='".$id_subitem."' ORDER BY num_sub_item");
	}
	...
	{	
		...
		if(count($array_link) == 1)//liberar botao Editar 
		{
			$edit_class = "";
			$edit = "";
		}
		...
		/*if(count($array_link) == 2)//liberar responsavel [R]
		{
			$setresponsa_class = "";
			$setresponsa = "";
		}
		...
		$setresponsa_class = "disabled";
		$setresponsa = "disabled='disabled'";
		if($row8["liberacao"]=='1')//liberar cronograma ativo
		{
			$edit_class = "disabled";
			$edit  = "disabled='disabled'";
			$setresponsa_class = "disabled";
		...
		if($row8["status_finalizar"]=='1')//finalizar cronogram ativo
		{
		...
		<td width='20'><input type='checkbox' name='codigo' class='codigo' value='".$row8["id"]."' liberacao='".$row8["liberacao"]."' /></td>
		<td style='font-size:10px; font-weight:bold; text-transform:uppercase !important;' height='45'>
			...
			<input type='button' name='set_responsa' class='set_responsa ".$setresponsa_class."' value='R' ".$setresponsa." />
		</td></tr>";
...
else echo "<div class='erro_geral'>ERRO NO CRONOGRAMA SUBITEM <br>Projeto ou Item n&atilde;o encontrados <br/>Entrar em contato [Eduardo Zambrano][eduardoz@clamom.com.br][TI]</div>";
?>
<div id="myObs">
	<div class="closeMenu"><img src="../../../../images/cronograma_all/x.png" /></div>
    <ul class="corpomyObs">
