...
$url = "http://192.168.0.190/erpclamom/cronograma_responsavel/index.php?projeto=".$_REQUEST["id_ctr"]."&item=0&subitem=".$codigo."&tipo=".$_REQUEST["tipo"]."&tipo2=ger&gere_coor=".$row_res["responsavel"]."&usuario=";						
//$url = "http://192.168.0.190/erpclamom/cronograma_responsavel/index.php?projeto=".$_REQUEST["id_ctr"]."&item=0&subitem=".$codigo."&tipo=".$_REQUEST["tipo"]."&usuario=".$row_res["responsavel"]."&tipo2=ger";
						//}
						//SALVAR NOVO SUBITEM												datainicio,datafim call numdias $row_tar["desc_tarefa"] $row_res["responsavel"]
					$sql="insert into tb_projeto_sub_itens(id_ctr,id_itens,id_tarefa,num_sub_item,descricao,data_inicio,num_dias,data_final,login,data_cadastro,nr_ctr,id_itens_SubComponente,ordem_tarefa,responsavel,liberacao,usu_gerente,url_cronograma) 
						values('".$_REQUEST["id_ctr"]."','".$row_item["id_itens"]."','".$row_deta["id_tarefa"]."','".$num_sub_item."','',now(),'0',now(),'".$_REQUEST["usuario"]."',now(),'".$_REQUEST["num_ctr"]."','".$IdSubComponente."','".$row_deta["ordem"]."','".$responsavel."','1','".$row_res["responsavel"]."','".$url."')";                               /*".strtoupper($row_desc["desc_item"])."*/
						mysql_query($sql);
