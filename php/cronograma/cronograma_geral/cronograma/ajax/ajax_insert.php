...
$sql="insert into tb_projeto_sub_itens(id_ctr,id_itens,id_tarefa,num_sub_item,descricao,data_inicio,num_dias,data_final,login,data_cadastro,nr_ctr,id_itens_SubComponente,ordem_tarefa,responsavel) 
	values('".$_REQUEST["id_ctr"]."','".$id_item."','".$row_deta["id_tarefa"]."','".$num_sub_item."','',now(),'0',now(),'".$_REQUEST["usuario"]."',now(),'".$_REQUEST["num_ctr"]."','".$IdSubComponente."','".($row_sub["ultimo"]+1)."','".$row_res["responsavel"]."')";
	mysql_query($sql);
	$id = mysql_insert_id();
