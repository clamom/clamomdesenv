...
{
	//pegar o primeiro resgistro 
	$resu_ger  = mysql_query("select * from tb_projeto_sub_itens where id_itens_SubComponente=".$_REQUEST["id_subitem"]." order by 1");
	$row_ger   = mysql_fetch_array($resu_ger);
$sql="insert into tb_projeto_sub_itens(id_ctr,id_itens,id_tarefa,num_sub_item,descricao,data_inicio,num_dias,data_final,login,data_cadastro,nr_ctr,id_itens_SubComponente,ordem_tarefa,responsavel,liberacao,usu_gerente) 
	values('".$_REQUEST["id_ctr"]."','".$id_item."','".$row_deta["id_tarefa"]."','".$num_sub_item."','',now(),'0',now(),'".$_REQUEST["usuario"]."',now(),'".$_REQUEST["num_ctr"]."','".$IdSubComponente."','".($row_sub["ultimo"]+1)."','".$row_res["responsavel"]."','".$row_res["liberacao"]."','".$row_ger["usu_gerente"]."')";
	$value_res = "R";//cronograma gerente
}
elseif($_REQUEST["tipo2"] == "coo")
{
$sql="insert into tb_projeto_sub_itens(id_ctr,id_itens,id_tarefa,num_sub_item,descricao,data_inicio,num_dias,data_final,login,data_cadastro,nr_ctr,id_itens_SubComponente,ordem_tarefa,responsavel,liberacao,usu_gerente,usu_coordenador,executor) 
	values('".$_REQUEST["id_ctr"]."','".$id_item."','".$row_deta["id_tarefa"]."','".$num_sub_item."','',now(),'0',now(),'".$_REQUEST["usuario"]."',now(),'".$_REQUEST["num_ctr"]."','".$IdSubComponente."','".($row_sub["ultimo"]+1)."','".$row_res["responsavel"]."','".$row_res["liberacao"]."','".$row_res["usu_gerente"]."','".$_REQUEST["usuario"]."','".$_REQUEST["usuario"]."')";
	$value_res = "E";//cronograma coordenador
}
	mysql_query($sql);
	$id = mysql_insert_id();
	//pegar responsavel do subitem pai
	$resu_res1 = mysql_query("select * from tb_projeto_sub_itens where id='".$id."' ");
	$row_res1  = mysql_fetch_array($resu_res1);
	//pegar usuario executor
	$resu_r2     = mysql_query("select *,upper(name) nome from sec_users where login = '".$row_res1["responsavel"]."' ");
	$row_r2      = mysql_fetch_array($resu_r2);
	$res_exe     = "[R. ".$row_r2["nome"]."]";
	if($row_res1["executor"] != "")
	{
		$resu_exe = mysql_query("select *,upper(name) nome from sec_users where login = '".$row_res1["executor"]."' ");
	...
	$td='<td width="20"><input type="checkbox" name="codigo" class="codigo" value="'.$id.'" /></td>
		...
		<input type="button" class="set_responsa '.$setresponsa_class.'" name="set_responsa" value="'.$value_res.'" '.$setresponsa.' style="display:none" />
		</td>';
	echo $td;
}
